...
**退出门槛**：Tile 尺寸完全一致 · 基本地面无缝重复 · 邻接规则可验证 ·
至少一张**引擎可加载、邻接合法**的示例地图 · Godot 与 Tiled 均可打开

> **第四条的措辞由 owner 于 2026-08-05 收窄**（原文是"至少一张**可玩**示例地图"）。
>
> 理由：owner 已于 2026-08-03 裁剪 Sprint 10，明确"本项目只做资产生成与自动化，
> **AI 驱动编辑器开发游戏不做**"。而"可玩"要求玩家、碰撞、目标与可运行关卡 ——
> 那是游戏开发，不是资产管线的退出条件。原措辞写在这条裁剪之前，与项目现有边界不一致。
>
> **收窄的是范围，不是标准。** 新措辞对应的证据同样是实打实的：`overworld.json`
> 邻接逐格验证通过，且在 Godot 4.7.1 headless 上逐格 `get_cell_atlas_coords` 读回一致。
> 8.6 在这条改动之前按**旧措辞**如实记过"未达成"（见 §8.6 的五条复核），那段记录原样保留 ——
> 翻这份文档的人应当看到"范围被 owner 收窄了"，而不是"门槛被人降了"。
...
#### 8.5 过渡 tile 与角落地形标注 · ✅ 已完成

本切补的是 Godot terrain bits 之前缺失的事实层：请求可给同质简写
`terrain: grass`，或按固定顺序
`[top_left, top_right, bottom_left, bottom_right]` 声明四角。凡被角落引用的地形，
必须至少有一块四角同质 base；没有参照物的请求直接拒收。未声明 terrain 的请求、
prompt、处理、Manifest 与 Mock 输出保持 8.1～8.4 路径，旧 Manifest 缺字段照常读取。

过渡 prompt 写明四角材质、内部边界，以及各外缘角段要接对应 base；四角同质时旧
prompt 逐字不变。Mock 的同质 tile 仍走旧渲染函数、返回字节不变；terrain 路径让
同质 base 先生成，再按 prompt 角落把对应 base 象限拼成过渡图，所以离线纵切也确实
有可量的像素关系，不是一张随机图加一句无人核对的声明。

##### 距离、分离度与 unknown

角落距离现在取**两条判据的较大值**，任一超过 `12.0` 都判不相容：第一条仍是逐
RGB 通道排序后等位像素绝对差的平均值，即三个一维边缘分布的 Wasserstein-1 距离；
它能消除随机颗粒位置差异，却不保留通道如何组合成联合 RGB 颜色。第二条在 6 个
双通道方向和 4 个三通道方向上投影 RGB，逐方向计算排序后的 Wasserstein-1 并取最大值，
专门抓第一条看不见的联合颜色成分变化。投影系数的绝对值和固定为 1，距离仍以 RGB
通道差为单位；给相邻调色板色重新分配质量只付相邻颜色的小代价，移到远色才付大代价。
这消除了总变差把“换了调色板桶”和“颜色真的移得很远”一视同仁的配置漂移。

这里选择固定投影 max-sliced Wasserstein，而不是每个象限求一次完整三维运输线性规划：
它保留 RGB 地面距离和 Wasserstein 的质量搬运含义，复杂度为 `O(N log N)`，适合在
每 tile × 每角 × 每 base 的验证热路径运行；固定方向又使结果确定、可复算。两条判据
都只除以固定的象限像素数，不使用颜色数、纹理颗粒度、方差或失败信号作为分母。

两条都刻意对象限内像素乱序不变：这里判的是“属于哪种地形”，不是纹理空间结构；
后者由 seam / adjacency 负责。审计反例实测为：`{红,绿}` 对 `{黄,黑}` 的逐通道
边缘分布完全相同，单判据为 `0.0`，联合判据加入后总距离为 `127.5`；同一草象限乱序
仍为 `0.0`（预期）；正常草对独立草样本为 `1.367188`，草对土为 `52.0`。把样本
每轴放大一倍后红绿/黄黑仍为 `127.5`。

更关键的稳健性证据来自真实 `process_tiles` 共享调色板量化链。两份独立草地采样的
配置扫描如下；“旧”是总变差实现，“新”是固定投影 max-sliced Wasserstein，余量为
`12.0 / 新读数`：

| tile_size | max_colors | 旧读数 | 新读数 | 阈值余量 |
|---:|---:|---:|---:|---:|
| 16 | 16 | 6.750000 | 2.554688 | 4.697× |
| 16 | 64 | 11.250000 | 2.552083 | 4.702× |
| 16 | 128 | 16.875000 | 2.708333 | 4.431× |
| 16 | 256 | 19.500000 | 2.755208 | 4.355× |
| 24 | 16 | 4.333333 | 1.479167 | 8.113× |
| 24 | 64 | 9.166667 | 2.335648 | 5.138× |
| 24 | 128 | 11.500000 | 2.532407 | 4.739× |
| 24 | 256 | 15.666667 | 2.784722 | 4.309× |
| 32 | 16 | 3.468750 | 0.816406 | 14.699× |
| 32 | 64 | 7.218750 | 1.535156 | 7.817× |
| 32 | 128 | 8.062500 | 1.638672 | 7.323× |
| 32 | 256 | 13.500000 | 1.611979 | 7.444× |

最终生产函数在 32px 合成样本（16×16 象限、独立同材质噪声 ±22）复测：

- 真过渡 `[grass, grass, dirt, dirt]`：`1.367188 / 1.128906 / 1.472656 /
  1.261719`，范围 `1.128906～1.472656`；
- 左上从 grass 换成 dirt：对声明 grass 的距离 `52.0`，最近地形改为 dirt，
  相对健康最大值分离 `35.310×`；
- 与任一 base 都不像的离群纹理：最近距离 `145.074219 / 144.152344 / 145.722656 /
  144.183594`，范围 `144.152344～145.722656`，相对健康最大值分离 `97.886×`。

工程绝对阈值定 `12.0`：跨 12 个合法配置的健康最大值为 `2.784722`，所以最坏仍有
`4.309×` 阈值余量，没有健康点超过阈值的四分之一；换角、红绿/黄黑与离群则分别高出
阈值 `4.333×`、`10.625×`、至少 `12.013×`。超过阈值记 unknown（Manifest/JSON 用
`null`，避免与合法地形名 `unknown` 冲突），绝不硬塞最近地形。样本仍是合成数据，
所以阈值随 Manifest 记为 `calibrated: false`，不冒充真实校准。

##### 产物、复算与导出

`create-tileset` 复用既有整套缩放 + 共享调色板量化链，在最终像素上执行纯函数
`derive_terrain_corners`；它不导入 provider、不调用 API。`TilesetInfo.terrain` 保存
阈值与校准状态，每块 `TileEntry.terrain` 保存 declared/measured 四角和四个最近距离。
`validate` 的 `tile_terrain` 用 Manifest 阈值从当前像素重算，逐角比声明与旧实测；
通过时同样记一笔。旧产物没有 terrain 段时是 `SKIP/not_applicable`。

`generic-json` 导出固定 `corner_order`、阈值、校准状态及逐 tile 的声明/实测/距离。
Godot 与 Tiled 导出器本切未改。

混合角 tile 的自接语义按 Manifest 四角布局**逐轴推导**：水平轴要求
`tl == tr 且 bl == br`，垂直轴要求 `tl == bl 且 tr == br`。因此上草下土只跳过
垂直轴，水平轴照常实测；SKIP 消息会列出轴、等式规则与实际角落剖面，不再用一句
“混合角不适用”整体逃掉闸门。`tile_border` 对所有 tile 一律运行。审计的颗粒度 8
探针中，合法上草下土水平 seam `1.080` 通过，垂直 seam `7.428305` 因布局不要求而
跳过，border `0.198286` 通过（阈值分别 `3.0` / `4.0`）；把边界改成左缘第 8 行、
右缘第 24 行后，水平 seam 升到 `3.933333` 并 FAIL。外缘能接谁继续由既有
`tile_adjacency` 像素重算：Mock 实测得到
`grass_base →down→ grass_dirt_corner →down→ dirt_base`。

这里必须标明 mock 的证据边界：`_render_transition_tile` 直接粘贴 base 的对应象限，
所以健康集成测试的四角距离精确为 `(0.0, 0.0, 0.0, 0.0)`。这只证明请求、生成、
处理、Manifest、复算和导出链路接通，**没有在集成层考验阈值 12.0 的判别力**；判别力
由上述带独立噪声的单元样本与三个反例承担。真实生成模型不会逐像素复制 base 象限，
不能据此推断真实过渡 tile 的健康距离也会是 0。

##### 三个真反例

干净 Mock 纵切 `create-tileset → validate → export generic-json` 实测：3 块 32×32
tile、16 色，12 个 `tile_terrain` 全 PASS；报告 `30 pass / 0 fail / 21 skip`，
Manifest 与导出 JSON 的过渡角均为声明 = 实测 `[grass, grass, dirt, dirt]`。

1. **A，换像素角。** 把过渡图左上 16×16 替换为 dirt base 对应象限，实际改动
   768 个通道值；该角对 grass/dirt 的候选距离从 `0/185.683594` 变为
   `185.683594/0`。`tile_terrain` FAIL 于 `grass_dirt_corner/top_left`：
   “声明 grass，像素实测 dirt”，实测最近距离 `0.0`、阈值 `12.0`；另三角仍 PASS。
2. **B，写反请求顺序。** 全新请求改成 `[dirt,dirt,grass,grass]`，注入一个忽略该
   顺序、仍画上草下土的 Mock，确保像素没有跟着错误声明一起变。Manifest 的 declared
   与 measured 分别为 `[dirt,dirt,grass,grass]` / `[grass,grass,dirt,dirt]`；四角
   全 FAIL，并逐角指出声明/实测差异，实测 `0.0`、阈值 `12.0`。
3. **C，最近也不够近。** 左上注入高亮噪声，对 grass/dirt 的距离为
   `232.457031/207.845703`，两者都超过 `12.0`。检查 FAIL 于
   `grass_dirt_corner/top_left`，实测 `207.845703`，明确写“像素实测 unknown”；
   没有把它硬塞为较近的 dirt。

实现过程中挖出并修掉的问题，按“只跑命令发现不了”排序：

1. **把混合角整体标成“不适用”会白白丢掉有牙齿的闸门。** 上草下土确实不能要求
   垂直自接，但水平自接实测稳定通过且能抓住左右不闭合的斜分界；border 也有二十倍
   余量。最终改为按四角剖面逐轴决定 seam，border 对所有 tile 保留。
2. **逐通道排序不是联合颜色分布。** `{红,绿}` 与 `{黄,黑}` 三个通道各自的边缘分布
   相同，单判据距离恰为 0。首版加入共享调色板计数总变差后虽为 24.0，却会随色数增加
   抬高同材质底噪；最终换成带 RGB 地面距离的固定投影 max-sliced Wasserstein，反例
   为 127.5、乱序仍为 0.0，12 个合法配置的同材质读数不超过 2.784722。
3. **B 若让 provider 忠实跟着写反的 prompt 画，根本不是反例。** 声明与像素一起
   翻转会一致通过；必须固定像素事实，只改变声明顺序，才真正检查角顺序契约。
4. **现有 planner 正确拒绝在既有 job 上换请求指纹。** 第一版 B 想复用 source 后
   改请求，被 input fingerprint 冲突拦住；改为全新请求 + 故意忽略顺序的 Mock，既不
   绕过 planner，也保住反例的单变量。
5. **已 export 的 job 不允许重新 validate。** 手工反例第一次复制了 exported 资产，
   状态机正确报“没有可验证任务”；最终三例均从只到 validated 的隔离基线执行，未把
   状态机拒绝误记成 terrain 检查结果。
6. **同一 terrain 有多块 base 时，prompt 与核验曾会选到不同参照。** prompt 原先按
   请求顺序覆盖成最后一块，像素核验则按 `tile_id` 取第一块；两边都确定性，却不是同一
   个事实。现在两处统一按 `tile_id` 取第一块，并用反序请求锁住。
7. **Mock 的过渡拼图不能只靠进程内缓存。** 断点若恰好停在 base 之后，续跑的新
   provider 实例拿不到旧缓存，过渡块会退回普通随机 tile。现在可从过渡 prompt 还原
   同质 base 的旧 prompt，并继续走原 `_render_tile`；新 provider 实例的象限逐像素
   等于原 base，有集成测试守着，未声明 terrain 的旧 Mock 字节路径不变。

##### 本地门禁实况（2026-08-04 沙箱返工）

- 按项目本体范围（明确不含未跟踪 `tools/promo/`）执行
  `uv run ruff check pixel_asset_forge tests tools/calibration tools/live-gate tools/godot-gate tools/tiled-gate scripts`：
  `All checks passed!`。
- `uv run mypy pixel_asset_forge`：
  `Success: no issues found in 83 source files`。
- 正式 `uv run pytest` 仍停在任务书预告的既有 asyncio pack 测试：前 23 项 PASS 后
  约 90 秒无新输出，没有失败；已中止该挂起进程，未删除、跳过或改写任何测试。
- 不排除任何测试，仅从 `/tmp/pixel-skill-asyncio/sitecustomize.py` 注入同步执行
  `asyncio.to_thread` 的沙箱补偿后，完整 1113 项收集结果为：
  `1108 passed, 5 skipped in 338.54s (0:05:38)`。相对第二轮审计新基线 `1107 / 5`
  净增本轮配置扫描 1 项、skip 不增；核心 terrain / seamless / transition 定向套件另为
  `24 passed in 3.4s`。补偿运行证明全部断言仍绿，但不冒充正常环境的线程池行为证据。

由于仓库 `.git` 在本沙箱被只读挂载，`feat/transition-tiles` 分支与提交也无法在这里
创建；正常审计环境仍须重跑正式三门禁并完成提交，不能把上面的补偿运行当成替代。

明确没做：Godot terrain set / peering bits、Tiled wangset、WFC 地形约束铺图；
`create-map` 行为未改。契约本身没有需要推翻的条款；8.1 自平铺判据只按角落剖面
豁免确实无定义的轴，边框判据不豁免。
...
4. **至少一张可玩示例地图：未达成。** 现有 `overworld.json` 是可复现且邻接合法的
   ...
   > **后续（2026-08-05）**：owner 据此把这条门槛的措辞收窄为"至少一张引擎可加载、
   > 邻接合法的示例地图"，理由见 §8 退出门槛处的决策记录 —— "可玩"所需的玩家 / 碰撞 /
   > 目标属于 Sprint 10 已被裁剪的游戏开发范围。**上面这段按旧措辞写下的"未达成"不删**：
   > 它记录的是 8.6 当时对着原标准的诚实自评，而不是一个被抹掉的结论。
   > 按新措辞，本条达成（证据同第 3 条与第 5 条）。
...
> **后续（2026-08-05）**：owner 随后把第四条的措辞收窄为"至少一张引擎可加载、
> 邻接合法的示例地图"（理由见 §8 退出门槛处的决策记录），Sprint 8 据此收官。
> 上面这段按旧措辞写下的"没有全部达成"**不删** —— 它是 8.6 当时对着原标准的诚实
> 自评。

#### 8.7 WFC 频率权重 · ✅ 已完成

8.3 记过一笔账：**"好看要等两样东西，都不在本切：过渡 tile 与频率权重"**。
8.5 交付了过渡 tile，本切收掉剩下那一半。

##### 先证明前置条件真的满足了

8.3 的困境是邻接表为对角矩阵（草只接草），网格连通 → 整张图必然单材质。
8.5 之后实测 `transition_tiles` 的邻接表：

```text
down: grass_base → [grass_base, grass_dirt_corner]
      grass_dirt_corner → [dirt_base]
```

**对角被打破了**，`grass → 过渡 → dirt` 是一条真实的垂直过渡链。所以"让不同材质
接得上"这个前置条件确实已经满足，本切不是在空中楼阁上加参数。

##### 但实测暴露出一个比"分布不自然"更根本的问题

跑 60 个 seed 的 8×6 地图：

| 指标 | 无权重 | 过渡块权重 6.0 | 压 base + 抬过渡 |
|---|---:|---:|---:|
| 完全单材质的地图 | **18%** | 2% | **0%** |
| 过渡 tile 占比均值 | 13.6% | 16.4% | 16.7% |

**过渡块明明可用，18% 的 seed 一次都没用它。** 原因是 `grass_base` 自己就满足
所有约束，均匀抽让这种"什么都不发生"的平凡解与其他解等概率。所以频率权重不是
锦上添花，它是**让过渡 tile 真正被用上的必要条件**。

##### 权重能做什么、不能做什么（这条边界要写清）

上表里过渡占比**顶到 16.7% 就不动了** —— 那正好是 8×6 的一整行（8/48）。
因为 `grass → 过渡 → dirt` 是一条**垂直**链，过渡带只能是一条水平线。
**权重减少的是平凡解，不能让过渡带变宽**；后者要靠给 tileset 加更多过渡块，
不是调参数。不把"平凡解从 18% 降到 0%"夸大成"地图变好看了"。

权重只影响塌缩时**抽哪个**候选，不参与相容性判定 —— 给 `water`（谁也接不上）
天大的权重也不能让它和别的 tile 相邻，有测试守着。

##### 为什么权重由人声明，不从邻接表反推

8.3 论证过"规则地图生成要人写意图，而我们只有邻接表"。权重正是**那种意图**，
从结构反推等于凭空发明用户没表达过的偏好。所以它落在 `TileSpec.weight`，
并随产物记进 `TileEntry.weight` —— `create-map` 读的是 Manifest 而不是请求，
不记就断链（同 8.2 记阈值、8.5 记 terrain 的理由）。

##### 实现过程中挖出并修掉的问题，按"只跑命令发现不了"排序

1. **等权也走 `random.choices` 会静默作废所有既有 seed。** `choices` 与 `choice`
   消耗随机数的方式不同，即使权重全相等，"顺手统一走 choices"也会让每个既有 seed
   产出另一张地图 —— 而"同 seed 同地图"是写进产物记录的承诺。所以无权重时逐位
   保持 `choice` 旧路径，并用一条测试盯住这个诱惑（实测反例：改走等权 `choices`
   后 seed 31 的地图确实变了）。
2. **CLI 那条单材质提示已经过时，而且会把人指向错的方向。** 旧提示一律说
   "缺的是过渡 tile"，那在 8.5 之前是对的；现在过渡块可用了，18% 的 seed 仍会
   铺出单色，此时让用户去补**已经存在的东西**就是误导。改为按邻接表结构分流：
   接不上材质 → 补过渡 tile；接得上 → 换 seed 或调 weight。判据取自邻接表结构
   （`transition_possible`），**不是**看这张图用了几种 tile —— 后者会把"这次恰好
   抽成单色"与"这套 tile 永远只能单色"混为一谈，而两者的建议正好相反。
3. **未知 tile 的权重必须拒收而不是忽略。** 静默忽略会让"我调了 weight 却没反应"
   变成查不出来的问题。全零权重则退回均匀抽 —— 那等于没表达偏好，不该让
   `random.choices` 抛 `ValueError`。
...
#### 9.4 真实资产质量闸门（live gate）· ✅ 已完成

**本次范围只有「拿真实模型跑一遍最小资产集，把产物质量收成量化数字并与阈值
比对」**。此前全部验收都建立在 mock 上 —— mock 证明的是**链路**（切帧、键控、
验证、导出的逻辑正确），证明不了**产物**（真实模型画出来的东西过不过得了自己的
验证引擎）。这两件事之间的缝，就是"测试全绿但用户拿到废图"的来路。

从本切起，**后续每个纵切的验收都要包含一次 live gate 通过**（改动不触及生成
链路的可豁免，豁免要写明理由）。

##### 预算：花钱的闸门必须先报价

真实调用产生计费。最小但有判别力的资产集：

| 资产 | 调用数 | 验什么 |
|---|---|---|
| `grass_field` tileset（3 tile） | 3 | 无缝两判据、共享调色板、邻接推导在真图上的表现 |
| `knight_01` seed | 1 | 键控、缩放、量化在真图上的表现 |
| `knight_01` walk_down 动画 | 1 | 切帧、锚点、帧序在真图上的表现 |
| **合计** | **5** | 每次运行的硬上限；脚本先打印预算再执行，超限即拒 |

##### 量化阈值（通过线）

| 指标 | 阈值 | 来源 |
|---|---|---|
| fatal / high 级失败数 | **= 0** | 验证引擎既有严重度 |
| 资产级通过率 | **3/3** | `ValidationReport.passed` |
| `tile_seam` 实测值 | ≤ 3.0 | §8.1 判据（顺带收真实样本，供校准欠账） |
| `tile_border` 实测值 | ≤ 2.0 | 同上 |
| `anchor_drift` 实测值 | ≤ 既有 per-action 阈值 | §9.1 |
| 调色板 | ≤ request 声明的 max_colors | 既有检查 |

medium / low 失败**不拦**，但要全部落盘 —— 它们正是阈值校准欠账（§9.1
未达标项 1）需要的真实样本。

##### 结果三态，不是两态

- **PASS** —— 全部达标
- **FAIL** —— 有 fatal / high，或资产级未全过
- **ERROR** —— 端点不通、Key 缺失、生成中断。**ERROR 不是 FAIL**：
  分不清这两者的闸门会把"网断了"记成"模型画不好"，校准数据就被污染了

##### 不进 CI

花钱 + 需要 Key，与 `tests/live/` 同口径：手动跑，报告落盘。

##### 退出门槛（仅对 9.4 主张）

- ✅ `tools/live-gate/` 落地：预算硬顶、三态结果、量化报告（JSON + Markdown）落盘
- ✅ 在真实端点上**实际跑通一次**，报告数字进仓库
- ✅ Key 不出现在任何落盘文件与日志里，有检查
- ✅ 报告里的每个指标都能对上验证引擎的检查项 id（不另造一套度量）
- ✅ Sprint 9.3 及之前全部既有测试不回归

**9.4 完成记录**：实现由 Codex 完成、经独立复核（我亲手重跑 ruff/mypy/单测/
mock 端到端，不采信转述）；真实端点两次运行，报告都在
`tools/live-gate/reports/` 里。

**第一次真实跑就 FAIL，而且 FAIL 得有价值**：`shallow_water` 的 `tile_border`
实测 2.178，越过当时的阈值 2.0（fatal）。人工裁决（3×3 平铺肉眼无缝、
边缘与中心落差仅 2.2 个色阶）判定为**误报**：真实健康 tile 的 border 分布
0.333–2.178，合成真失败在 11.7+ —— 原阈值切在健康分布内部。这正是 §9.1
"阈值未用真实样本校准"欠账的第一次兑现。

处置：`TILE_BORDER_DEVIATION_MAX` 2.0 → 4.0（健康最大值 1.8× 余量、距真失败
2.9×），n=3，**仍不标记校准完成**。顺带修掉一个漂移隐患：live gate 原本把阈值
复制成本地常量，改成从验证引擎导入 —— 否则改引擎阈值后闸门还在拿旧数判。

校准后离线重验原产物（validate 不花钱）三块 tile 全过；真实跑 #2 **PASS**：
资产 3/3、fatal/high 0、量化指标 10/10。两份报告（一 FAIL 一 PASS）都进仓库 ——
闸门两个方向都真的动过。

跑 #2 实测数字（gpt-image-2）：tile_seam 0.58–0.94、tile_border 0.24–2.05、
anchor_drift 0.39px、三份调色板全部顶格但不超（16/16、32/32、32/32）。
视觉抽查：草/水平铺无缝，骑士 8 帧身份一致、脚底齐线；dirt_path 偏素净
（prompt 丰富度问题，不是闸门问题，不拦）。

编排注记：并行 worktree 里的 Codex 任务会被其沙箱的只读挂载卡死（两个代理的
失败行为都正确 —— 拒绝在脏主仓库动手、拒绝造假交付）。Codex 任务改为主工作区
串行。

> 9.4 只主张"真实模型的产物质量有了量化判据与第一份实测数据"。
...
Owner 人工审图判定当日校准特效样本不合格；按不对称校准策略，这批样本从阈值证据
中排除，`constants.py` 的阈值不作任何修改。`loop` / `travel` 旧节拍曾直接命令整个
sprite 缩放，与它们的尺寸变化阈值自相矛盾；现改为外缘范围稳定、由内部能量流、
亮度轮转和消散形成逐帧差异，并用零成本单测阻止尺寸变化命令词回流。

旧节拍一律写 `the shape`，没有任何一处说明这个特效**是什么**，于是"闪电链"退化成
无身份的通用星爆。现在由 compiler 把"这个特效是什么"与"外缘范围和亮像素量在各帧
间不变"提升为**一个公共块只说一次**，而不是逐格重复：区分各格的只有节拍里那十几个
词，重复的公共前缀会把它们埋掉；描述一旦跨行还会破坏
`Cell N (row R, column C): ...` **每格一行**的格式契约 —— 而只数
`startswith("Cell ")` 恰恰抓不住跨行（首行照样以 `Cell` 开头、行数照样等于帧数），
所以格式契约另有断言核对姿势块内每一行都是 Cell 记录。

**本切只改契约矛盾并加确定性守卫，mock 只验证全链可复现；没有调用 live API，真实
观感待 live 复跑确认，不主张画质已经改善。**
