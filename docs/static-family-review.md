...
## 7. 真实动画链路复审（2026-08-02，gpt-image-2）

§6 只跑了静态资产。本节补动画链路：`knight_01` 的 canonical seed →
人工闸门 → `walk_down` 8 帧动作网格，另加一个 96px 对照组。共 4 次调用。

### 7.1 证实的假设（PLAN §7 的待验证清单）

- **A-6「seed 作参考图（不用 mask）足以保持身份」—— 成立，且成立得很干净。**
  8 帧全部是同一个角色：棕发、绿披风、皮甲、右手持剑，服装配色与比例
  零漂移。这是本项目最难的问题，真实数据上表现优秀。
- **端点尺寸吸附再次实锤，且数字与文档完全一致**：请求 `2048×1024`
  实际返回 **`1774×887`** —— 与 PLAN §2.3 记录的实测值一字不差。
  seed 请求 `1024×1024` 返回 `1254×1254`。
- **像素网格吸附两条分支都走到了**：seed 判定吸附（块 7.5px，
  1254×1254 → 167×167，前景色数 **50884 → 2339**）；动作网格判定
  「本来就是 1:1 像素画」保持原图。这条 README 称为"产出可用像素资产的
  前提"的链路，mock 下从未真正执行过。
- **抽帧与几何全部达标**：8 帧、越界干净、锚点漂移 0.5px（48px）/
  0.4px（96px），验证 14 项通过 12 跳过 2、零失败。

### 7.2 关键发现：`target_size: 48` 对带武器人形角色不够 —— 示例本身配错了

流水线会告警「模型原生画出约 330 逻辑像素高，目标画布只有 48px，
约 85% 的细节会被丢掉，建议 target_size 提到 96 或更高」。
**这条告警是可信的**，用同一个 seed 跑 96px 对照组实测：

| | 48px | 96px |
|---|---|---|
| 脸 | 糊成一团，看不出五官 | 眼睛、表情都在 |
| 剑 | 一条细线 | 剑柄与剑刃分明 |
| 走路的迈步动作 | **看不出来**（接近 Sprint 0 A-2 的"八张站姿"失败模式） | 帧 2 一腿前一腿后，跨步清晰 |
| 身份一致性 | 好 | 同样好（无损失） |

也就是说，48px 下那个"腿不动"的观感**不是 prompt 失败，是分辨率装不下**。

`examples/knight.yaml` 用的正是 48px —— 照示例做的用户会拿到次优资产。
已改为 96 并写明实测依据。`slime.yaml`（简单对称角色）与 `fireball.yaml`
（特效）细节密度低得多，保持 48 不变。

### 7.3 顺带实测：`spell_bundle` 契约依赖的闸门与续跑机制可用

`create-character` 产出后确实停在 `awaiting_approval` 并打印人工闸门提示；
`create-animation --approve-seed` 一次批准解锁 8 个动画任务，
命令行输出还列出了"还有 7 个动画任务可跑"。PLAN §7.4 采用的路线
（seed 闸门保留 + 靠既有断点续跑承接）在真实链路上验证可行。

### 7.4 仍未覆盖

四方向身份一致性（只跑了 `down`）· A-7（`up` 背面）· 镜像 derive ·
真实的修复循环（本轮零失败，Repair Planner 未被触发）· Godot 真机 ·
并发/退避压测。
