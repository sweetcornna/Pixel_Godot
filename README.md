...
两条边界不能省略：`up` ×1.3 系数**仍未被验证**（三个 `walk_up` 样本实测远低于
基准阈值，系数没派上用场，既没被证实也没被证伪）；`anchor_drift` 这批实测
0.36～0.54px，而 16 个样本里有 8 个格线落在非整数位、端到端引入约 0.137 逻辑像素的
系统性误差（占测量值近 28%），**在切帧取整修复落地并复跑前不得据此收紧**。
