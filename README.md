...
矩阵可一条命令生成、量测、聚合并列出 contact sheet/GIF。

**2026-08-05 的 live 运行已完成**（`gpt-image-2`，25/25 对账通过，量化证据入库在
`tools/calibration/reports/20260805T060618.921814Z/`），实测到 `loop` / `travel` /
`cast` 共四组指标越过现阈值。但**阈值一个都没改**：`constants.py` 的 SHA-256 运行
前后一致，而按不对称策略，放宽必须先逐个审核 contact sheet/GIF 确认样本合格 ——
**这道人工闸门尚未走**，所以现在只有候选证据，不是已完成的校准。

一条边界不能省略：`up` ×1.3 系数**仍未被验证** —— 三个 `walk_up` 样本实测远低于
基准阈值，系数没派上用场，既没被证实也没被证伪。

关于 `anchor_drift`：这批 16 个样本里有 8 个格线落在非整数位，一度判断"收紧要等
切帧取整修复后复跑"。修复（`center_crop_to_grid`）落地后用**同一批原图**离线重跑
处理复测，**该判断已收回** —— 决定收紧的最大值只从 0.4995 动到 0.4984。同一次补测
确认四组越线结论全部成立，**校准证据对切帧改动是稳健的，不需要为此重跑**。
详见[阈值校准记录](docs/threshold-calibration.md)。
...
**频率权重。** 请求里每块 tile 可给 `weight`（默认 1.0）。它**只影响塌缩时抽哪个
候选，不参与相容性判定**——任何权重都不会让一条非法接缝变合法。它存在是因为均匀抽
会大量落进平凡解：实测 60 个 seed 的 8×6 地图有 **18% 完全单材质**（过渡块可用却
一次没用），把过渡块权重提到 6.0 后降到 **2%**，再压低两块 base 可到 **0%**。

> **权重改不了过渡带的宽度。** 同一实测里过渡 tile 占比顶到 16.7% 就不动了——
> 那正好是 8×6 的一整行，因为 `grass → 过渡 → dirt` 是一条**垂直**链，过渡带只能
> 是一条水平线。要更宽的过渡区，缺的是**更多过渡块**，不是调参数。

单材质地图有两种成因，`create-map` 会分流提示，不会把人指向错的方向：

- **邻接表是对角矩阵**（材质之间接不上）→ 网格连通，整张图必然同一种材质，
  这是**正确结果**；缺的是过渡 tile，不是更好的求解器，也不是调 `weight`。
- **接得上但这次恰好抽成单色** → 换 `--seed` 或给过渡块调高 `weight`。

多材质求解能力由 `tests/unit/test_wfc.py` 用合成邻接表验证（真实的对角矩阵上，
"每对相邻格是否合法"会恒真，那等于没查）。
