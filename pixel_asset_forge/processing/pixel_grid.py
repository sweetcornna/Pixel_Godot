...
引入 anchor sheet 之后，模型照着一张近 1:1 的参考作画（anchor sheet 把 seed
缩到 0.22× 平铺进格子，块 6 变成块 1.3，块结构在参考图里就没了），
产出因此也接近 1:1，块检测的还原误差普遍在 25 上下、越过阈值被拒绝。

一度以为这是回归。实测同一张源图三种处理，答案相反：

```text
不吸附            最锐利，脸/弓/靴子都可读
强制吸附 块=5     明显变糊，细节成片丢失
强制吸附 块=4     介于两者之间，仍不如不吸附
```

**高分辨率的近 1:1 产出，直接下采样比先按臆想的块还原再下采样更干净。**
还原误差这道门正确地区分了两类源：真有平块的（早期 knight 网格，误差 6.7）
吸附，没有的（archer，误差 26）不吸附。
"""
