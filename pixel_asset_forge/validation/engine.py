...
#: 无缝判据的阈值。`seam` 仍是工程默认值；`border` 已用第一批真实样本修正过一次。
#:
#: 合成 tile 实测分离度：可平铺 0.96/0.19，渐变 31.67，带边框 11.69，暗角 11.84。
#: 2026-08-03 的 live gate（gpt-image-2，PLAN §9.4）给出第一批真实样本：
#: **肉眼无缝**的三块地面 tile，border 实测 0.333 / 1.589 / 2.178 —— 原阈值 2.0
#: 切在了健康分布内部，把 shallow_water 误判成暗角（边缘与中心的逐通道落差只有
#: 2.2 个色阶，<1%，平铺后肉眼看不出任何网格线；只是水面颗粒度太细把比值抬过线）。
#: 上调到 4.0：健康最大值 2.178 的 1.8 倍余量、距真失败 11.69 还有 2.9 倍。
#: 样本量 n=3，**仍不算校准完成**（§9.1 口径不变），后续 live gate 继续积累。
TILE_SEAM_RATIO_MAX = 3.0
TILE_BORDER_DEVIATION_MAX = 4.0
...
def validate_static_image(
    ...
    *,
    ...
    #
    # 判据是"主体接触画布边缘 → 可能已被裁切"。静态链把内容按
    # ``fit = min(画布宽/内容宽, 画布高/内容高)`` 等比缩放后居中放回 ——
    # 内容**恒完整落在画布内**，顶边只意味着"刚好铺满"，从不意味着被切掉。
    # 检查再去问"碰到边缘了吗"，问的是我们自己刚做完的那步算术。
    #
    # 实测（2026-08-06）：把原图整张涂成不透明实心 —— 最极端的"该接触边缘"
    # 情形 —— content_bounds 照样 PASS。同一趟里 ``cell_overflow`` 正确判 FAIL：
    # 真正的裁切信号在**原图**上，那里主体是否顶边由模型决定，不由我们决定，
    # 所以那个检查是有内容的。
    #
    ...
    # 检查项仍在报告里可见，但不再冒充覆盖。
    #
    # **也不能改成"顶边即 FAIL"**：2026-08-07 拆掉静态链那 1px 内缩之后，
    # 顶边成了正常产出（6 张真实产出里钥匙就顶满 24px，且验证全过）——
    # 那条规则会把合格图直接误杀。这一项的 SKIP 因此比拆内缩之前更必要。
    checks.append(
        Check.make(
            "content_bounds",
            target,
