...
def process(
    ...
    原始生成图永不覆盖，所以对齐、缩放基准、键控阈值都可以在这里反复重跑，
    一分钱都不用花。

    **画布尺寸与调色板改不动**：这两项以 Manifest 为准（幂等性要求，见
    ADR-004），改 `request.yaml` 里的 `target_size` / `max_colors` 重跑无效 ——
    真要换得重新生成。检测到这类改动会打警告，不会静悄悄忽略。
    """
    _load_config(config_path)
    try:
