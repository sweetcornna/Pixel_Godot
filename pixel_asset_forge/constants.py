...
#:
#: `chunky_icon` 的数值来自对 Kenney *Pixel Platformer*（CC0，231 张 18×18 单体图）
#: 的实测分布，2026-08-06 量得：
#:
#: | 量 | 中位 | 10–90 分位 |
#: |---|---|---|
#: | 每张色数 | 4 | 2 – 6 |
#: | 明度层级 | 4 | 2 – 6 |
#: | 主体占画布 | 0.83 | 0.33 – 1.00 |
#: | 轮廓比内部亮度差 | −32.6 | −74.4 – 0 |
#:
#: 取 24 而不是 18：18 不在 `LOGICAL_SIZES` 里，而**相邻档位取大的那个** ——
#: `processing/pixel_grid.py` 实测模型原生逻辑分辨率下角色约 55px 高，
#: 目标定得越小丢的产出越多（那份文档写明 32×32 已是"丢掉一半模型产出"）。
#: 6 色取的是分布上沿而非中位，同理：宁可多留一档给量化器,也不要在生成阶段
#: 就把可读性压死。
