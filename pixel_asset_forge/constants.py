...
#: 命名风格档位：一组**实测标定过**的 style 字段组合。
#:
#: 存在的理由：`target_size` / `max_colors` / `shading` / `outline` 得配套才成风格，
#: 而这四个此前只能逐个猜。档位把"某种商用资产长什么样"固化成可复用的一行。
#:
#: **档位只填空缺** —— 请求里显式写了的字段一律优先，档位绝不覆盖（见
#: `StyleSpec._apply_preset`）。
#:
#: `chunky_icon` 的数值来自对 Kenney *Pixel Platformer*（CC0）的实测分布。
#: **总体选错过一次，2026-08-06 真实生成验证时发现并改正**，两份都列在这里，
#: 因为"该拿哪一批图当基准"正是这次唯一实质性的错误：
#:
#: | 量 | `Tiles/*.png`（n=180，**错**） | `Tiles/Characters/*.png`（n=27，**对**） |
#: |---|---|---|
#: | 画布边长 | 18 | **24** |
#: | 每张色数 | 4（2 – 6） | 7（5 – 8） |
#: | 明度层级 | 3.5（2 – 5） | 5（4 – 6） |
#: | 主体占画布 | 0.83（0.33 – 1.00） | 0.62（0.30 – 0.82） |
#: | 轮廓比内部亮度差 | −29.5（−58.8 – −11.1） | −38.3（−53.7 – −0.9） |
#:
#: `Tiles/` 那 180 张是**瓦片**：大半是泥土/草地/水面这类 2–4 色的地形块，
#: 道具图元还压着一层深色瓦片底板。拿它的中位数去标定"单体道具图标"，
#: 等于让地形块替道具投票 —— 色数被压低、占画布被底板抬高。
#: `Tiles/Characters/` 那 27 张才是**独立精灵**：真透明背景、无底板，
#: 与本档位要产出的东西同类。
#:
#: 改正后两个数反而更站得住：
#:
#: * **24 不是折中，就是 Kenney 自己独立精灵的边长。** 原先写"18 不在
#:   `LOGICAL_SIZES` 里所以相邻取大"，那是拿瓦片尺寸当依据绕出来的理由；
#:   真实依据是 Kenney 的非瓦片精灵本来就画在 24×24 上。
#: * **6 色落在正确总体的 5 – 8 区间内**（偏下沿）。原先写"取分布上沿"，
#:   那是相对错总体（2 – 6）说的；相对对的总体它其实偏保守。
#:
#: 真实生成实测（gpt-image-2，9 次调用，6 档位 + 3 同主体对照）：档位产出
#: 6 色 / 24px，同主体不给档位的对照产出 11 色 / 32px，Kenney 基准 7 色 / 24px
#: —— 档位确实把产出拉到了基准邻域，且这个位移由档位而非主体造成。
STYLE_PRESETS: Final[dict[str, dict[str, object]]] = {
    "chunky_icon": {
        "target_size": (24, 24),
        "max_colors": 6,
        "shading": "flat",
